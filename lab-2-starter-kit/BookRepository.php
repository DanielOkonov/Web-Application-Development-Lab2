...
class BookRepository
{
    ...
    public function deleteBookByISBN(string $isbn): void
    {
        ...
        $books = array_filter($books, function (Book $book) use ($isbn) {
            return $book->getInternationalStandardBookNumber() !== $isbn;
        });

        $this->writeToFile(array_values($books));
    }

    private function writeToFile(array $books): void
    {
        $jsonBooks = json_encode($books, JSON_PRETTY_PRINT);
        file_put_contents($this->filename, $jsonBooks);
    }
